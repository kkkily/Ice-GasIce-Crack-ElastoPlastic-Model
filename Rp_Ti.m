...
n=size(Ti,1);
for i=1:1:n
[L4(:,i),L5(:,i),L6(:,i),Pi(:,i),taui(:,i),Pa(:,i),t]=refs(a,b,dt,Ti(i,1),sw1,sa,sw2);
...
KIITi=KII(n,:)';
for i=1:1:n
    RpTi(:,i)=rp(KITi(i,1),KIITi(i,1),oys);
end
RpTi=real(RpTi);
figure
sigam=[0:1:360]'.*(pi/180);
polarplot(sigam,RpTi(:,1)) %a=10mm
hold on 
polarplot(sigam,RpTi(:,41))%a=20mm
hold on 
polarplot(sigam,RpTi(:,81))%a=30mm
hold on 
polarplot(sigam,RpTi(:,121))%a=40mm
